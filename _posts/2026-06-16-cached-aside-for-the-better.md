...
My instinct was to obsess over the cache itself. I had assumed our traffic was Zipfian — that a small number of users would account for a large share of the lookups — and reasoned that a cache which was clever about keeping the "hot" users would win. So I reached for [Ristretto](https://github.com/dgraph-io/ristretto), which is genuinely sophisticated about what it admits and evicts: a Count-Min Sketch to estimate frequency, and TinyLFU for eviction. (The [dgraph write-up on its design](https://discuss.dgraph.io/t/introducing-ristretto-a-high-performance-go-cache-dgraph-blog/5102) is an excellent read.)

Here I have to own a mistake, because it shaped everything that followed. The traffic *was* roughly Zipfian in aggregate, but I had quietly conflated two different things. A skewed distribution of requests *across all users* does not mean that any single instance sees the same user often enough to cache them usefully. The popularity skew was real; the per-instance locality I'd assumed it implied was not. And because of that, the cache's cleverness was almost beside the point — something far simpler, like [FreeCache](https://github.com/coocood/freecache), would have done nearly as well. (When we did eventually A/B test the two, Ristretto edged FreeCache out — but by a surprising 2–4%, which only confirmed that the cache was never where the real leverage lay.)
...
What genuinely kept me up was staleness. A cache can serve old data, and in a safety system "old data" can quietly mean "letting through something you should have caught." So we couldn't simply pick a cache size and move on — we had to sit down as a team and work through it score by score: how stale is too stale for this one? An expiry that is entirely safe for one signal is reckless for another.
...
There's a coda, and it's probably the part I learned the most from. I wanted to come back, and I was wanted back — the whole team advocated for it, and the leader our tech lead reported to fought hard to make it happen. But the decision didn't sit with the people I'd actually worked alongside; it sat with people who hadn't, and for reasons that were more political than anything to do with me, it didn't happen.
