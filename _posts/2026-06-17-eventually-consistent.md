...
Besides picking up a little exposure to [Envoy](https://www.envoyproxy.io/), [Envoy Gateway](https://gateway.envoyproxy.io/), [Istio](https://istio.io/), and the [xDS protocol](https://www.envoyproxy.io/docs/envoy/latest/api-docs/xds_protocol), the piece I have been learning most from is mundane and, to me, fascinating: getting a binary I wrote to ship reliably. Rather than have a pipeline reach out over the network for it each time, I publish the binary to an internal **artifact repository** — somewhere that hands back the exact same immutable version on every request, which is what makes a build [hermetic](https://bazel.build/basics/hermeticity) and reproducible — and then bake that pinned binary into a container image at build time:
...
The image itself is assembled declaratively — a base image plus a thin layer carrying my binary — using the kind of tooling that builds images from packages rather than from a hand-written Dockerfile (the [Wolfi](https://github.com/wolfi-dev) / [apko](https://github.com/chainguard-dev/apko) world). The original motivation was to move dependency-fetching out of the pipeline's runtime and into the build itself, cutting down on the ad-hoc network calls we made every time CI spun up. From there, the binary publishes the rendered configuration files up to our control plane, which in turn distributes them out to its subscribers.

To a senior engineer, baking a binary into a container is probably routine, but as an associate it was genuinely one of the most exciting things I have done: create and upload the binary, work out how to bake it as a thin layer into the container image, and then have the CI agent run that image. It is these small learning experiences that make me grateful to be a young associate engineer — I am allowed to make mistakes, and there is so much still to learn. I do wonder how much that will change as I progress through my career.
...
That is what this blog is for: to intertwine the technical and the personal, to consolidate what I learn, and to keep an honest record of the convergence. I was told the door was closed, partly because of AI. I now spend my days using AI to do work I love, at a company I was lucky enough to be asked back to. Everything, it seems, happens for a reason — and the only reliable way I have found to deserve the good outcomes is to stay hungry, stay reflective, and keep doing my absolute best while the state of the world slowly catches up.